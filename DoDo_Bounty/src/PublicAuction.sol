...
contract PublicAuction {
    ...
    function bid() external payable {
        ...
        require(
            block.timestamp >= lastBidTime[msg.sender] + cooldownPeriod || lastBidTime[msg.sender] == 0,
            "Cooldown period not passed"
        );

        // 当在拍卖最后5分钟内有人竞拍，则需要加权出价并延长拍卖时间
        if (auctionEndTime - block.timestamp <= 5 minutes) {
            uint weightedBid = highestBid * 110 / 100; // 加权出价（按1.1倍计算）
            require(msg.value >= weightedBid, "Weighted bid not high enough");
